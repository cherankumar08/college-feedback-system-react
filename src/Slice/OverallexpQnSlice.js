...
export const deleteQuestion = createAsyncThunk(
  "overallquestions/deleteQuestion",
  async (questionId) => {
    await axios.delete(`http://localhost:5000/overallexp/${questionId}`);
    return questionId;
  }
);
...
export const updateQuestion = createAsyncThunk(
  "overallquestions/updateQuestion",
  async (updatedQuestion) => {
    for (let id in updatedQuestion) {
      await axios.patch(`http://localhost:5000/overallexp/${id}`, {
...
export const fetchCourseQuestions = createAsyncThunk(
  "overallquestions/fetchCourseQuestions",
  async () => {
    const response = await fetch("http://localhost:5000/overallexp");
    const data = await response.json();
    return data;
  }
);

const OverallexpQnSlice = createSlice({
  name: "overallquestions",
  initialState: {
    questions: [],
    status: "idle",
    error: null,
...
export default OverallexpQnSlice.reducer;
