...
export const deleteQuestion = createAsyncThunk(
  "campusfacilities/deleteQuestion",
  async (questionId) => {
    await axios.delete(
      `http://localhost:5000/campusfacilities/${questionId}`
    );
    return questionId;
  }
);
...
export const updateQuestion = createAsyncThunk(
  "campusfacilities/updateQuestion",
  async (updatedQuestion) => {
    for (let id in updatedQuestion) {
      await axios.patch(`http://localhost:5000/campusfacilities/${id}`, {
...
export const fetchCourseQuestions = createAsyncThunk(
  "campusfacilities/fetchCourseQuestions",
  async () => {
    const response = await fetch("http://localhost:5000/campusfacilities");
    const data = await response.json();
    return data;
  }
);

const CampusFacilitiesSlice = createSlice({
...
export default CampusFacilitiesSlice.reducer;
