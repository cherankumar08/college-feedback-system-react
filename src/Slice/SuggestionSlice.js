...
export const deleteQuestion = createAsyncThunk(
  "surveyOnSuggestion/deleteQuestion",
  async (questionId) => {
    await axios.delete(
      `http://localhost:5000/surveyOnSuggestion/${questionId}`
    );
    return questionId;
  }
);
...
export const updateQuestion = createAsyncThunk(
  "surveyOnSuggestion/updateQuestion",
  async (updatedQuestion) => {
    for (let id in updatedQuestion) {
      await axios.patch(
        `http://localhost:5000/surveyOnSuggestion/${id}`,
        {
...
export const fetchCourseQuestions = createAsyncThunk(
  "surveyOnSuggestion/fetchCourseQuestions",
  async () => {
    const response = await fetch(
      "http://localhost:5000/surveyOnSuggestion"
    );
