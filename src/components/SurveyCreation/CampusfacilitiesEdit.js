...
} from "../../Slice/CampusFacilitiesSlice";

const CampusfacilitiesEdit = () => {
  const dispatch = useDispatch();
  const questions = useSelector((state) => state.campusfacilities.questions);
  const status = useSelector((state) => state.campusfacilities.status);
  const error = useSelector((state) => state.campusfacilities.error);
  ...
  const headings = ["Library", "Laboratory", "Support Services"];
  ...
  return (
    <div className="m-5">
      ...
      <p className="h5 text-center my-5">Campus Facilities Questions</p>
...
export default CampusfacilitiesEdit;
