...
} from "../../Slice/ExtracurricularSlice";

const ExtracurricularEdit = () => {
  const dispatch = useDispatch();
  const questions = useSelector(
    (state) => state.extracurricularActivities.questions
  );
  const status = useSelector((state) => state.extracurricularActivities.status);
  const error = useSelector((state) => state.extracurricularActivities.error);
  ...
  const headings = ["Events", "Clubs", "Support Services"];
