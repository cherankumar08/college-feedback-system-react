...
} from "../../Slice/OverallexpQnSlice";

const OverallExpEdit = () => {
  const dispatch = useDispatch();
  const questions = useSelector((state) => state.overallquestions.questions);
  const status = useSelector((state) => state.overallquestions.status);
  const error = useSelector((state) => state.overallquestions.error);
  ...
  const headings = ["Campus Facilities", "Overall Experience"];
  ...
  return (
    <div className="m-5">
      ...
      <p className="h5 text-center my-5">Overall Questions</p>
