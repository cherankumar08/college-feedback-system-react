...
} from "../../Slice/instructorSurveyQnSlice";

const InstructorSurveyEdit = () => {
  const dispatch = useDispatch();
  const questions = useSelector((state) => state.instructorquestions.questions);
  const status = useSelector((state) => state.instructorquestions.status);
  const error = useSelector((state) => state.instructorquestions.error);
  ...
  return (
    <div className="m-5">
      ...
      <p className="h5 text-center my-5">Instructor Survey Questions</p>
...
export default InstructorSurveyEdit;
