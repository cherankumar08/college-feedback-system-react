...
} from "../../Slice/CampusServicesSlice";

const CampusServicesEdit = () => {
  const dispatch = useDispatch();
  const questions = useSelector((state) => state.campusServices.questions);
  const status = useSelector((state) => state.campusServices.status);
  const error = useSelector((state) => state.campusServices.error);
  ...
  return (
    <div className="m-5">
      ...
      <p className="h5 text-center my-5">Campus Services Questions</p>
...
export default CampusServicesEdit;
